...
- 新增 ETL 式 `buildEstimatedSchedules`：以車站倒數、站點方向、地理距離與估算速度建立局部 `ESTIMATED` 行進段，讓觀測車輛可在地圖上平滑移動。
- 推估行進明確保留方法與距離／秒數 metadata，不視為官方列車位置。
- 車站查詢改為路線分頁與車站按鈕選擇器，移除不適合控制中心操作的原生 `select`。
- 每個車站各自快取 60 秒；一分鐘內切回不重新請求，並依經過時間衰減快取倒數，避免頻繁存取第三方來源。
