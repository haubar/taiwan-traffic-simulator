...
export function useTrainPosition(lines,schedules,simSec){
  const activeTrains=computed(()=>{
    ...
    for(const seg of schedules.value ?? schedules){
      ...
      grouped.set(key,{...seg,progress:p,x:a.x+(b.x-a.x)*p,y:a.y+(b.y-a.y)*p,fromName:a.name,toName:b.name,departureTime:formatClock(seg.departureSec),arrivalTime:formatClock(seg.arrivalSec),updatedAt:new Date().toISOString()})
    }
