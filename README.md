...
平面圖與 3D 地圖可由工具列切換。3D 視圖使用 Three.js 建立地形、道路、建築區塊、站體、軌道與立體車輛；可拖曳平移／旋轉、滾輪縮放。切換「跟車旅程」後，鏡頭會跟隨目前選取的列車（未選取時跟隨第一列運行中列車），車輛依模擬時間沿站間路徑平滑前進，並可看到站體與沿線環境。現階段底圖是依北部路網拓撲建立的數位分身，不是 GPS 實景、地圖圖磚或 CAD 車輛模型；後續可替換為授權的 GIS／3D 地圖圖層。
