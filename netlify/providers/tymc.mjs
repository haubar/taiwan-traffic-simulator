...
export const parseInterstationCsv = (csv) => {
  ...
  return rows.slice(1).map((row) => ({ routeCode: row[route], vehicleType: sources.vehicleTypes[row[type]] ?? row[type], sequence: row[serial], fromStation: row[from], toStation: row[to], seconds: Number(row[seconds]) })).filter((row) => row.fromStation && row.toStation && Number.isFinite(row.seconds) && row.seconds > 0)
}
